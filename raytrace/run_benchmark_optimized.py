...
class Vector(object):
    ...
    def __add__(self, other):
        # Flattened type check: assume Vector + Vector in the hot path
    ...
    def reflectThrough(self, normal):
        ...
        d2 = 2.0 * (x * nx + y * ny + z * nz)
        return Vector(x - d2 * nx, y - d2 * ny, z - d2 * nz)
...
class Sphere(object):
    ...
    def intersectionTime(self, ray):
        # Extract primitive coordinates to do raw math
        cx, cy, cz = self.centre.x, self.centre.y, self.centre.z
        rpx, rpy, rpz = ray.point.x, ray.point.y, ray.point.z
        rvx, rvy, rvz = ray.vector.x, ray.vector.y, ray.vector.z

        # cp = self.centre - ray.point
        cpx, cpy, cpz = cx - rpx, cy - rpy, cz - rpz

        # v = cp.dot(ray.vector)
        v = cpx * rvx + cpy * rvy + cpz * rvz

        # cp.dot(cp)
        cp_sq = cpx * cpx + cpy * cpy + cpz * cpz

        discriminant = (self.radius * self.radius) - (cp_sq - v * v)
        if discriminant < 0:
            return None
        return v - math.sqrt(discriminant)

    def normalAt(self, p):
        # Inlined (p - self.centre).normalized()
        x = p.x - self.centre.x
        y = p.y - self.centre.y
        z = p.z - self.centre.z
        inv_mag = 1.0 / math.sqrt(x * x + y * y + z * z)
...
class Halfspace(object):
    ...
    def intersectionTime(self, ray):
        # Inlined ray.vector.dot(self.normal)
        v = (ray.vector.x * self.normal.x +
             ray.vector.y * self.normal.y +
             ray.vector.z * self.normal.z)
        if v:
            return 1.0 / -v
        return None
...
class Ray(object):
    ...
    def pointAtTime(self, t):
        # Inlined self.point + self.vector.scale(t)
        return Point(self.point.x + self.vector.x * t,
                     self.point.y + self.vector.y * t,
                     self.point.z + self.vector.z * t)    
